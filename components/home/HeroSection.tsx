...
export default function HeroSection() {
  ...
  return (
    <section className="relative min-h-screen flex flex-col px-4 sm:px-6 overflow-hidden zavo-bg-clean">
      <div className="hero-glow"></div>
      <div className="hero-bg"></div>

      {/* Mobile Navigation */}
      <nav className="flex md:hidden items-center justify-between w-full pt-6 pb-4 relative z-20">
        <Link href="/" className="text-xl font-bold hover:opacity-80 transition-opacity">
          <span className="text-blue-600">Lynlytics</span>
        </Link>
        <a
          href="mailto:anoopsai@lynlytics.com"
          className="px-4 py-2 rounded-full bg-[#1877F2] text-white text-sm font-semibold hover:bg-[#0d66d0] transition-colors shadow-sm"
        >
          Contact
        </a>
      </nav>
      ...
      <div className="flex-1 flex justify-center items-center">
        <div className="max-w-5xl mx-auto text-center relative z-10 py-8 sm:py-12">
          ...
          {/* Lyn's Central Floating Bar - Mobile Responsive */}
          <div className="mt-6 sm:mt-8 mb-6 sm:mb-8">
            <div className="bg-white/80 backdrop-blur-[20px] border border-blue-200/50 rounded-full shadow-lg shadow-blue-500/15 w-full max-w-[90vw] sm:w-[720px] h-[60px] sm:h-[65px] mx-auto flex items-center justify-between px-3 sm:px-5">
              <div className="orb-refined">
                <div className="orb-glow"></div>
              </div>
              <div className="status-text text-left flex-1 ml-3 sm:ml-4">
                <p className="text-xs sm:text-sm font-semibold text-slate-800 fade-text m-0">
                  {floaterMessages[currentMessage].l1}
                </p>
                <p className="text-xs text-slate-500 fade-text m-0 hidden sm:block">
                  {floaterMessages[currentMessage].l2}
                </p>
              </div>
              <div className="ml-2 sm:ml-4 flex items-center gap-1 sm:gap-2">
                <button className="p-1.5 sm:p-2 rounded-full hover:bg-blue-100 transition">
          ...
          <p className="mt-4 sm:mt-6 max-w-3xl mx-auto text-black text-base sm:text-lg md:text-xl leading-relaxed font-normal font-[var(--font-geist-sans)] px-4">
          ...
          <div className="mt-8 sm:mt-10">
            <form
              ...
              method="POST"
              className="flex flex-col sm:flex-row gap-3 sm:gap-4 items-center justify-center max-w-md mx-auto px-4"
            >
              <input
                type="email"
                name="email"
                ...
                required
                className="w-full sm:flex-1 px-4 sm:px-6 py-3 sm:py-4 rounded-full bg-white border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#1877F2] focus:border-transparent text-gray-900 placeholder-gray-400 border shadow-none text-sm sm:text-base"
              />
              <button
                type="submit"
                className="w-full sm:w-auto magnetic-btn rounded-full bg-[#1877F2] text-white font-semibold text-sm sm:text-base hover:bg-[#0d66d0] transition-all inline-block whitespace-nowrap shadow-lg shadow-blue-500/25 bg-foreground px-6 sm:px-8 py-3 sm:py-4"
              >
                Join Waitlist
              </button>
            </form>
          </div>
          <div className="mt-8 sm:mt-12 text-base sm:text-lg md:text-xl">
            <span className="opacity-100 text-black">The intelligence engine for modern SMB&apos;s like </span>
            <span
              id="smb-animation"
              className="gradient-violet font-medium inline-block transition-opacity duration-300 min-w-[200px] sm:min-w-[240px] text-center opacity-100"
            >
              {currentIndustry}
            </span>
          </div>
