...
export default function FAQSection() {
  ...
  return (
    <section id="faq" className="py-12 sm:py-16 md:py-20 px-4 sm:px-6 zavo-bg-pure">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-12 sm:mb-16 md:mb-20">
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-gray-900 mb-4">
            FAQs
          </h2>
          <p className="text-base sm:text-lg text-gray-600 px-4">Everything you need to know about Lynlytics</p>
        </div>
          ...
          {faqs.map((faq, index) => (
            <div key={index} className="group border-t border-gray-200">
              <button
                ...
                className="w-full py-6 sm:py-8 flex items-start justify-between text-left transition-colors hover:border-gray-300"
              >
                <h3 className="text-base sm:text-lg md:text-xl font-semibold text-gray-900 pr-4 sm:pr-8 leading-tight">
                  {faq.question}
                </h3>
                <svg
                  className={`w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 text-gray-900 flex-shrink-0 transition-transform duration-300 ${activeIndex === index ? "rotate-45" : ""}`}
                  viewBox="0 0 30 30"
                  fill="currentColor"
                >
              ...
              >
                <p className="text-sm sm:text-base text-gray-600 pb-6 sm:pb-8 whitespace-pre-line leading-relaxed">
                  {faq.answer}
                </p>
              </div>
            </div>
